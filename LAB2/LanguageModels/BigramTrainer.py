...
class BigramTrainer(object):
    ...
    def process_token(self, token):
        ...
        if token not in self.word:
            self.word[token] = self.unique_words
            self.index[self.unique_words] = token
        ...
        if self.total_words != 1:
            lastWord = self.index[self.last_index]
            self.bigram_count[lastWord][token] += 1
    ...
    def stats(self):
        ...
        for word in self.unigram_count:
            index = str(self.word[word])
            occurrences = self.unigram_count[word]
            rows_to_print.append(index + " " + word + " " + str(occurrences))

        for word in self.unigram_count:
            for innerKey in self.bigram_count[word]:
                index = str(self.word[word])
                occurrences = self.unigram_count[word]
                innerIndex = self.word[innerKey]
                pairOccurrences = self.bigram_count[word][innerKey]
                p = math.log(pairOccurrences / occurrences)
                p = "{0:.15f}".format(p)
                rows_to_print.append(str(index) + " " + str(innerIndex) + " " + str(p))
